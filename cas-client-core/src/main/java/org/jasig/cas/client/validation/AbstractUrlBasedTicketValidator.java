...
public abstract class AbstractUrlBasedTicketValidator implements TicketValidator {
    ...
    /**
     * Prefix for the CAS server. Should be everything up to the url endpoint,
     * including the /.
     *
    ...
    /**
     * Disable XML Schema validation. Note, setting this to true may not be
     * reversable. Defaults to false. Setting it to false after setting it to true
    ...
    protected final String constructValidationUrl(final String ticket, final String serviceUrl) {
        ...
        // 子类实现
        // Cas10TicketValidator 返回 validate
        final String suffix = getUrlSuffix();
        final StringBuilder buffer = new StringBuilder(
            urlParameters.size() * 10 + this.casServerUrlPrefix.length() + suffix.length() + 1);
     ...
     * @param url
     *            the url to encode.
     * @return the encoded url, or the original url if "UTF-8" character encoding
     *         could not be found.
     */
    ...
    public Assertion validate(final String ticket, final String service) throws TicketValidationException {
        // 构造检验 ticket 请求，用于通过 http 请求检验 ticket 是否有效
